...
if (u2==NULL) {
	sprintf(text,"\nSex        : %c\nLevel      : %s\nLast login : %s",u->sex,level_name[u->level],ctime((time_t *)&(u->last_login)));
	write_user(user,text);
...
idle=(int)(time(0) - u->last_input)/60;
sprintf(text,"\nSex         : %c\nLevel       : %s\nIgnoring all: %s\n",u->sex,level_name[u->level],noyes2[u->ignall]);
write_user(user,text);
...
if (strncmp(user->room->maptype,"bib")) {
...
if (word_count<2) {
	write_user(user,"usage: .room [open/link/map/desc/status/save] <other info...>\n");
        sprintf(text,"\n"); 
	return;
	}
