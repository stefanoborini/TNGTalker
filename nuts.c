...
************************************************/
	...
	while(user!=NULL) {
		...
		if (user->type==REMOTE_TYPE) {  user=next;  continue; }
...
fscanf(fp,"%d",&user->num_of_clones);
for (i=0;i<user->num_of_clones; i++) fscanf(fp,"%s\n",user->clone_name[i];
fclose(fp);
return 1;
}
...
fprintf(fp,"%c\n",user->sex);
fprintf(fp,"%d\n",user->num_of_clones);
for (i=0;i<user->num_of_clones;i++) fprintf(fp,"%s\n",user->clone_name[i]);
fclose(fp);
return 1;
}
...
sprintf(filename,"%s/%s.",USERFILES,user->name);
...
for(i=0;i<MAX_CLONES;++i) user->clone_name[i][0]='\0';
return user;
}
...
sprintf(filename,"%s/%s.log",LOGDIR,SYSLOG);
...
if (word_count<2) {
	write_user(user,"Usage : .clone <clone name>\n");
	return;
	}


for (u=user_first; u!=NULL;u=u->next) if (u->owner==user) cnt++;

if (cnt==max_clones) {
	write_user(user,"Sorry... you can't create another clone\n");
	return;
	}
...
if (word_count<2) {
	if(user->level>MAGHETTO) {
		write_user(user,"usage: .room [open/link/map/desc/status/save/reload] <other info...>\n");
	        sprintf(text,"\n"); 
		return;
		}
	if (user->level==MAGHETTO) {
...
sprintf(text,"Now you are at level : ~OL%s\n",level_name[user->path][user->level]);
...
if (word_count<2) {
	write_user(user,"usage: .levellist <path number>\n\n");
